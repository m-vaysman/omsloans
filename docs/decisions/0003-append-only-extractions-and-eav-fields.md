...
The costs are storage and query complexity. Raw JSON for every attempt, retained indefinitely, plus a new row per reprocessing run, means the tables grow monotonically and a retention policy will eventually be needed. Reading current values requires filtering on `IsCurrent`, and getting that wrong silently returns superseded data — so exactly one current extraction per notice is enforced transactionally rather than by convention. EAV gives up the type safety and check constraints that real columns would provide: field names are only as consistent as the prompt that emits them, which is why the field-name convention is pinned in [`docs/extraction-fields.md`](../extraction-fields.md) and enforced by a single flattener shared across providers rather than left to each one. That convention is a path — `events[0].economics.principal_amount` — rather than a fixed vocabulary per notice type, because one notice can state several economic events and the set of names is therefore open. Reporting queries pivot rather than select, and the typed projection columns exist so that numeric and date comparisons do not degrade into string comparison.
