<#
.SYNOPSIS
    Stops the OmsLoan review API service.

.DESCRIPTION
    Stops the service and waits for it to report Stopped.

    The wait matters: the host is configured with a 20-second shutdown timeout so in-flight
    review requests can drain, and the SCM logs a service that overruns as a crash. Waiting
    here distinguishes a clean stop from one that was killed.

    Stopping this service does not affect ingestion. OmsLoanWorker keeps running and keeps
    filling the queue; reviewers simply cannot reach it until this is started again.

.EXAMPLE
    .\Stop-OmsLoanApiService.ps1
#>
[CmdletBinding()]
param(
